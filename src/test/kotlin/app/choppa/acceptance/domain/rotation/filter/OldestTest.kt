...
import app.choppa.domain.rotation.filter.oldest
...
internal class OldestTest {
     ...
     * |  6     | [{M1,M2},{M3,M4,M5},{}] | 3       | [{M1,M2},{M3},{}]  |
     */
    @ParameterizedTest
    @MethodSource("oldestTestArgs")
    fun `Given squad candidates ordered by oldest to newest member in the squad, when rotation filter oldest is applied to collect an amount of candidates, then oldest filter should return expected candidates`(
        candidates: List<List<Member>>,
        amount: Int,
        expected: List<List<Member>>,
    ) {
        oldest(candidates, amount) shouldBeEqualTo expected
    }

    companion object {
        @JvmStatic
        fun oldestTestArgs(): Stream<Arguments?>? {
            return Stream.of(
                // Test 1
