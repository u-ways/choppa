...
class SmartMemberRotation(
    ...
    private val memberToCurrentSquadMap: HashMap<Member, Squad> = hashMapOf<Member, Squad>().apply {
        squads.map { squad -> squad.members.filter { it.active && it.chapter == chapter }.map { mem -> Pair(mem, squad) } }.flatten().forEach { this[it.first] = it.second }
    }
    ...
    fun invoke(): List<MoveToSquad> {
        ...
        val rotationAmount = floor(tempSquadCandidatesToRotate.size.toDouble() / 2).toInt().coerceAtMost(amount)
        (0 until rotationAmount).forEach { _ ->
    ...
    fun List<Squad>.findCandidatesToRotate(chapter: Chapter): MutableList<SquadDefinition> = map {
        Pair(it, it.members.toList().filter { x -> x.chapter == chapter && x.active }) // && x.squads.size > 1
    ...
    private fun calculateKSP(spp: Double, mpp: Double): Double = (spp * sppScalar) + (-mpp * mppScalar)
